...
위 그림에서는 `5x5x1` 입력 이미지에 대하여 `3x3x1` filter를 1개 사용하여 한칸씩 움직이며(`stride=1`) 매번 이미지의 일부와 필터 간 matrix operation을 수행하여 최종적으로 `3x3` feature map을 추출합니다. 이때 matrix operation, 즉 각 요소끼리 곱하고 이를 모두 더하는 연산을 convolution이라고 합니다.
...
컬러 이미지는 Red, Green, Blue 총 3개의 채널을 갖는 3차원 데이터입니다.

![]({{site.url}}/assets/images/DL/CV/CNN-3d-convolution.png)


`MxNx3` 입력 이미지에 대하여 convolution을 수행할 때 사용되는 filter는 `KxKx3`입니다.


예를 들어, Red, Green, Blue 총 3개의 채널을 갖는 컬러 이미지에 대하여 `3x3` filter를 3개 사용하여 convolution을 수행한다고 가정해봅시다. `MxNx3` 입력 이미지에 대하여 `3x3x3` filter를 사용하여 convolution을 수행하는 과정은 다음과 같습니다.
...
즉, 각각의 채널 별로 입력 이미지와 filter 간 matirx operation을 수행한 뒤 이들을 모두 더하여 1개의 채널을 갖는 feature map을 반환합니다.
