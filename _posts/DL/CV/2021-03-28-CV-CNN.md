...
convolution을 수행할 때, filter가 사용됩니다. CNN에서 filter는 학습 대상 파라미터로, backpropagation을 통해 filter 내 값들을 학습합니다.
...
- **stride** : stride는 filter를 몇 칸씩 움직이면서 convolution을 수행할 것인지를 의미합니다.
  - stride 값이 커지면 중복되는 부분이 줄어들고 convolution을 수행하는 범위가 줄어들어 local feature의 특성을 고려하지 못한 global feature를 생성해낼 가능성이 높습니다.
...
예를 들어, 아래와 같이 `4 x 4 x 1` 이미지가 있다고 가정해봅시다.
